 ...
 */
const PU = require('./preferences-util')
...
class PreferencesSettings {
  ...
  static localizeSystemPanel() {
    ...
    if (note) { note.textContent = this.defS.system.notes }
  }

  static localizeKeybindingsPanel() {
    const info = this.sv.querySelector('.keybinding-panel>div:nth-child(2)')
    if (!PU.isAlreadyLocalized(info)) {
      info.querySelector('span:nth-child(2)').textContent = this.defS.keymaps.notes.text1
      info.querySelector('span:nth-child(4)').textContent = this.defS.keymaps.notes.text2
    ...
    const searchBarPlaceholder = this.sv.querySelector('.keybinding-panel>div:nth-child(3) .placeholder-text')
    if (!PU.isAlreadyLocalized(searchBarPlaceholder)) {
      searchBarPlaceholder.textContent = this.defS.keymaps['search-bar-placeholder']
      searchBarPlaceholder.setAttribute('data-localized', 'true')
    }

    const keyBindTableHeader = this.sv.querySelector('.keybinding-panel>table thead')
    if (!PU.isAlreadyLocalized(keyBindTableHeader)) {
      keyBindTableHeader.querySelector('th.keystroke').textContent = this.defS.keymaps['table-header-keystroke']
      keyBindTableHeader.querySelector('th.command').textContent = this.defS.keymaps['table-header-command']
      keyBindTableHeader.querySelector('th.source').textContent = this.defS.keymaps['table-header-source']
      keyBindTableHeader.querySelector('th.selector').textContent = this.defS.keymaps['table-header-selector']
  ...
  static localizeThemesPanel() {
    ...
    if (!PU.isAlreadyLocalized(info)) {
      ...
      tp1.querySelector('.setting-description').textContent = this.defS.themes.description1
      ...
      tp2.querySelector('.setting-description').textContent = this.defS.themes.description2
      info.setAttribute('data-localized', 'true')
    }
  }

  static localizeUpdatesPanel() {
    PU.applyTextWithOrg(this.sv.querySelector('.update-all-button.btn-primary'), this.defS.updates['update-all'])
    PU.applyTextWithOrg(this.sv.querySelector('.update-all-button:not(.btn-primary)'), this.defS.updates['check-updates'])
    PU.applyTextWithOrg(this.sv.querySelector('.alert.icon-hourglass'),  this.defS.updates['checking-updates'])
    PU.applyTextWithOrg(this.sv.querySelector('.alert.icon-heart'),  this.defS.updates['all-updated-message'])
  ...
  static localizeInstallPanel() {
    ...
    if (!PU.isAlreadyLocalized(info)) {
      info.querySelector('span:nth-child(2)').textContent = this.defS.install.notes.text1
      const tc = info.querySelector('span:nth-child(4)')
      tc.textContent = tc.textContent.replace('and are installed to', this.defS.install.notes.text2)
      const span = document.createElement('span')
    ...
    if (!PU.isAlreadyLocalized(searchContainer)) {
      PU.applyTextWithOrg(inst.querySelector('.search-container .btn:nth-child(1)'), this.defS.install['btn-packages'])
      PU.applyTextWithOrg(inst.querySelector('.search-container .btn:nth-child(2)'), this.defS.install['btn-themes'])
