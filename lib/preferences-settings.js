...
class PreferencesSettings {
  ...
  static localizeKeybindingsPanel() {
    ...
    if (!PU.isAlreadyLocalized(info)) {
      info.querySelector(
        'span:nth-child(2)'
      ).textContent = this.defS.keymaps.notes.text1;
      info.querySelector(
        'span:nth-child(4)'
      ).textContent = this.defS.keymaps.notes.text2;
    ...
    if (!PU.isAlreadyLocalized(searchBarPlaceholder)) {
      searchBarPlaceholder.textContent = this.defS.keymaps[
    ...
    if (!PU.isAlreadyLocalized(keyBindTableHeader)) {
      keyBindTableHeader.querySelector(
        'th.keystroke'
      ).textContent = this.defS.keymaps['table-header-keystroke'];
      keyBindTableHeader.querySelector(
        'th.command'
      ).textContent = this.defS.keymaps['table-header-command'];
      keyBindTableHeader.querySelector(
        'th.source'
      ).textContent = this.defS.keymaps['table-header-source'];
      keyBindTableHeader.querySelector(
        'th.selector'
      ).textContent = this.defS.keymaps['table-header-selector'];
  ...
  static localizeThemesPanel() {
    ...
    if (!PU.isAlreadyLocalized(info)) {
      ...
      tp1.querySelector(
        '.setting-description'
      ).textContent = this.defS.themes.description1;
      ...
      tp2.querySelector(
        '.setting-description'
      ).textContent = this.defS.themes.description2;
      info.setAttribute('data-localized', 'true');
    }
  }
  ...
  static localizeInstallPanel() {
    ...
    if (!PU.isAlreadyLocalized(info)) {
      info.querySelector(
        'span:nth-child(2)'
      ).textContent = this.defS.install.notes.text1;
