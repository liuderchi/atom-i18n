class PreferencesUtil {
  ...
  static isAlreadyLocalized(elem) {
    let localized
    if (elem) { localized = elem.getAttribute('data-localized') }
  ...
  static applyButtonToolbar() {
    ...
    for (var btn of sv.querySelectorAll('.meta-controls .install-button')) {
      if (!this.isAlreadyLocalized(btn)) {
        ...
        const buttonText = btn.textContent.startsWith('Update to')
          ? this.defS['control-btns'].update + (version = btn.textContent.substr(9))
    ...
    for (btn of sv.querySelectorAll('.meta-controls .icon-playback-pause span')) {
      this.applyTextWithOrg(btn, this.defS['control-btns'].disable)
    }
    for (btn of sv.querySelectorAll('.meta-controls .icon-playback-play span')) {
