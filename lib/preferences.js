...
const PreferencesSettings = require('./preferences-settings')

class Preferences {

  static delaySettings() {
    let settingsEnabled
    const settingsTab = document.querySelector('.tab-bar [data-type="SettingsView"]')
    if (settingsTab) { settingsEnabled = settingsTab.className.includes('active') }
    if (!settingsTab || !settingsEnabled) { return }
    try {
      this.sv = document.querySelector('.settings-view')

      // NOTE i18n tab-title, placeholder before checking window.I18N.pref.done
      settingsTab.querySelector('.title').textContent = this.defS.Settings['tab-title']
      this.applyPlaceholders()
      if (window.I18N.pref.done) { return }
      ...
      // Add Events
      const btns = this.sv.querySelectorAll('div.section:not(.themes-panel) .search-container .btn')
      for (let btn of btns) {
  ...
  static applyPlaceholders() {
    let searchBarPlaceholder = this.sv.querySelector('.keybinding-panel>div:nth-child(3) .placeholder-text')
    if (searchBarPlaceholder) {
      searchBarPlaceholder.textContent = this.defS.Settings.keymaps['search-bar-placeholder']
    }
    this.sv.querySelectorAll('section.section .editor-container .placeholder-text').forEach(div => {
    ...
    searchBarPlaceholder = this.sv.querySelector('div.section:not(.themes-panel) .search-container .placeholder-text')
    if (searchBarPlaceholder) {
      searchBarPlaceholder.textContent = this.defS.Settings.install['searchbar-placeholder']
    }
  }
  ...
  static applyLeftSide() {
    ...
    for (let d of this.defS.Settings.menu) {
      ...
      if (el) { PU.applyTextWithOrg(el, d.value) }
    }
